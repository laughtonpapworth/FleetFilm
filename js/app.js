...
async function submitFilm(){
  ...
  let picked = null;
  try{
    let res = await omdbSearch(title, year);
    let candidates = (res && res.Search) ? res.Search.filter(x=>x.Type==='movie') : [];

    // If OMDb struggles with punctuation / accents / alt spellings, retry with a normalised title (and without year).
    if(!candidates.length){
      const norm = normalizeTitleForSearch(title);
      if(norm && norm.toLowerCase() !== title.toLowerCase()){
        res = await omdbSearch(norm, year);
        candidates = (res && res.Search) ? res.Search.filter(x=>x.Type==='movie') : [];
      }
    }
    if(!candidates.length){
      const norm = normalizeTitleForSearch(title);
      res = await omdbSearch(norm || title, null);
      candidates = (res && res.Search) ? res.Search.filter(x=>x.Type==='movie') : [];
    }
...
async async function exportNextProgrammeCsv(){
  try{
  ...
  els.nextProgList.innerHTML = `
    <div class="form-grid" style="margin-bottom:8px">
      ...
      <div class="span-2 notice" style="margin-top:0">
        Tip: Next Programme is meant to hold the 9 films for the next cycle. Archiving stores the programme date on each film for reporting.
      </div>
    </div>
  `;
