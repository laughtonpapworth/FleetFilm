/* Fleet Film App – Passport-centric, TMDB-powered
   Nav: submit | films | green | nextprog | discarded | archive | calendar | addresses
   All film detail/voting/status lives inside the Film Passport modal.
*/
...
function getTmdbKey() {
  return (window.__FLEETFILM__CONFIG && window.__FLEETFILM__CONFIG.tmdbApiKey) || '01ad889e5a067c30e081abc72e5f93c7';
}
...
async function tmdbDetailsById(id) {
  // id can be a TMDB numeric id or an IMDb tt-id
...
function extractUkRating(detail) {
  ...
  const gb = rd.results.find(r => r.iso_3166_1 === 'GB');
  if (gb && gb.release_dates && gb.release_dates.length) {
...
function tmdbToFilmDoc(detail, credits) {
  const posterUrl = detail.poster_path ? TMDB_IMG + detail.poster_path : '';
  const language = detail.spoken_languages?.map(l => l.english_name || l.name).filter(Boolean).join(', ') || detail.original_language || '';
  const country = detail.production_countries?.map(c => c.name).filter(Boolean).join(', ') || '';
  const genre = detail.genres?.map(g => g.name).filter(Boolean).join(', ') || '';
  const runtime = detail.runtime || null;
  const synopsis = detail.overview || '';
  const year = detail.release_date ? parseInt(detail.release_date.slice(0, 4), 10) : null;
  const ukAgeRating = extractUkRating(detail);
...
function showTmdbPicker(results, initialQuery) {
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `
      <div class="modal" role="dialog" aria-label="Select film">
    ...
    const renderResults = (items) => {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="tmdb-no-result"><div class="tmdb-no-result-icon">🎬</div><div>No results found. Try a different spelling or use "Add manually".</div></div>`;
        return;
      }
      list.innerHTML = '';
      items.forEach(it => {
        const year = it.release_date ? it.release_date.slice(0, 4) : '?';
        const poster = it.poster_path ? `<img src="${TMDB_IMG}${it.poster_path}" alt="poster" class="poster-small">` : '<div class="poster-small" style="display:flex;align-items:center;justify-content:center;font-size:20px;">🎬</div>';
        const langs = it.original_language ? `<span style="text-transform:uppercase;font-size:10px;background:var(--gold-soft);color:var(--gold);padding:2px 6px;border-radius:4px;">${it.original_language}</span>` : '';
        ...
        row.innerHTML =
          ...
          `<div class="modal-row-main">
            ...
            <div class="modal-row-sub">${year} ${langs} ${it.vote_count > 0 ? `· ★ ${it.vote_average?.toFixed(1)}` : ''}</div>
    ...
    const doSearch = async () => {
      ...
      try {
        const newResults = await tmdbSearch(q, null);
        renderResults(newResults);
      } catch (e) {
        overlay.querySelector('#picker-list').innerHTML = '<div class="notice">Search failed. Try again.</div>';
      }
    };
  ...
  films.forEach(f => {
    if (Array.isArray(f.screenings) && f.screenings.length) {
      f.screenings.forEach(sc => {
        ...
        const loc = sc.locationName ? ` · ${sc.locationName}` : '';
        const time = sc.time ? ` ${sc.time}` : '';
        (byISO[sc.dateISO] ||= []).push({ text: `${f.title}${time}${loc}`, id: f.id });
      });
      return;
    }
...
async function ensureUserDoc(u) {
  ...
  state.role = (rs.exists && rs.data()?.role) || 'member';
...
function attachHandlers() {
  ...
  document.getElementById('btn-email-signin')?.addEventListener('click', async () => {
    try { await auth.signInWithEmailAndPassword(document.getElementById('email').value, document.getElementById('password').value); } catch (e) { alert(e.message); }
  });
  document.getElementById('btn-email-create')?.addEventListener('click', async () => {
    try { await auth.createUserWithEmailAndPassword(document.getElementById('email').value, document.getElementById('password').value); } catch (e) { alert(e.message); }
  });
  ...
  document.getElementById('tab-more')?.addEventListener('click', () => {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    overlay.innerHTML = `<div class="modal"><div class="modal-head"><h2>Pages</h2><button class="btn btn-ghost" id="more-close">✕</button></div><div class="modal-list" id="more-list"></div></div>`;
    document.body.appendChild(overlay);
...
function extractTmdbOrImdbId(input) {
  const s = String(input || '').trim();
  if (!s) return { type: null, id: null };
  // TMDB URL: themoviedb.org/movie/12345
  const tmdbUrl = s.match(/themoviedb\.org\/movie\/(\d+)/);
  if (tmdbUrl) return { type: 'tmdb', id: tmdbUrl[1] };
  // Pure TMDB numeric id
  if (/^\d{3,8}$/.test(s)) return { type: 'tmdb', id: s };
  // IMDb tt-id or URL
  const imdb = s.match(/(tt\d{6,10})/i);
  if (imdb) return { type: 'imdb', id: imdb[1].toLowerCase() };
...
async function submitFilm(opts = { mode: 'search' }) {
  ...
  const rawId = (document.getElementById('f-imdb')?.value || '').trim();
  ...
  if (mode === 'manual') {
    ...
    await addFilmDoc({ title, year, tmdbData: null }); return;
  }

  // Direct ID lookup
  const parsed = extractTmdbOrImdbId(rawId);
  if (parsed.type) {
    try {
      const res = await tmdbDetailsById(parsed.id);
      if (!res) { if (!confirm('Could not find that film on TMDB. Add manually?')) return; await addFilmDoc({ title, year, tmdbData: null }); return; }
      await addFilmDoc({ title: res.detail.title || title, year: null, tmdbData: res }); return;
    } catch (e) {
      if (!confirm('TMDB lookup failed. Add manually?')) return;
      await addFilmDoc({ title, year, tmdbData: null }); return;
    }
  }

  if (!title) { alert('Enter a film title to search.'); return; }

  // Search TMDB
  try {
    const results = await tmdbSearch(title, useYear && year ? year : null);
    // If no results with year, retry without
    let candidates = results;
    if (!candidates.length && year) candidates = await tmdbSearch(title, null);

    const choice = await showTmdbPicker(candidates, title);
    if (choice.mode === 'cancel') return;
    if (choice.mode === 'manual') { await addFilmDoc({ title, year, tmdbData: null }); return; }
    if (choice.mode === 'pick') {
      const res = await tmdbDetailsById(choice.tmdbId);
      await addFilmDoc({ title, year: null, tmdbData: res });
    }
  } catch (e) {
    console.error('TMDB search failed:', e);
    if (!confirm('Could not reach TMDB. Add manually?')) return;
    await addFilmDoc({ title, year, tmdbData: null });
  }
}

async function addFilmDoc({ title, year, tmdbData }) {
  const base = {
    title: title || '',
    year: year,
    synopsis: '',
    ...
    runtimeMinutes: null,
    language: '', ageRating: '', ukAgeRating: '',
  ...
  if (tmdbData) {
    const mapped = tmdbToFilmDoc(tmdbData.detail, tmdbData.credits);
    Object.assign(base, mapped);
  }

  const ref = await db.collection('films').add(base);
  await logAudit(ref.id, 'create', { via: tmdbData ? 'tmdb' : 'manual' });

  const msg = document.getElementById('submit-msg');
  if (msg) { msg.textContent = `"${base.title}" added!`; msg.classList.remove('hidden'); setTimeout(() => msg.classList.add('hidden'), 2500); }
...
async function loadFilms() {
  ...
  films.sort((a, b) => {
    const mode = filterState.sort;
    const tA = (a.title||''), tB = (b.title||'');
    const yA = typeof a.year==='number' ? a.year : 0, yB = typeof b.year==='number' ? b.year : 0;
    ...
    if (mode==='created-asc')  return cA - cB;
    return cB - cA;
  });
...
function filmRowCard(f) {
  ...
  const poster = f.posterUrl
    ? `<img alt="Poster" src="${escapeHtml(f.posterUrl)}" class="film-row-poster" style="width:66px;height:99px;object-fit:cover;border-radius:8px;border:1px solid var(--border);" loading="lazy">`
    : '<div class="poster-placeholder">🎬</div>';
  const origTitle = f.originalTitle && f.originalTitle !== f.title
    ? `<div style="font-size:12px;color:var(--ink-3);font-style:italic;margin-top:2px;">${escapeHtml(f.originalTitle)}</div>` : '';
...
function simpleFilmCard(f, extraHtml = '') {
  const year = f.year ? `(${f.year})` : '';
  const poster = f.posterUrl ? `<img alt="Poster" src="${escapeHtml(f.posterUrl)}" style="width:66px;height:99px;object-fit:cover;border-radius:8px;border:1px solid var(--border);" loading="lazy">` : '<div class="poster-placeholder">🎬</div>';
...
async function openPassport(f) {
  ...
  const hasStream = wts.includes('Stream'), hasDisk = wts.includes('Disk'), hasScreen = wts.includes('Screen');
          ...
          <div class="passport-status-block">
            ...
            <div id="pp-green-wrap" style="margin-top:6px" class="${['greenlist','next_programme','archived'].includes(f.status)?'':'hidden'}">
              ${f.greenAt && typeof f.greenAt.toDate==='function'
                ? `<span class="badge badge-green" style="font-size:11px;">🌿 Green since ${f.greenAt.toDate().toISOString().slice(0,10)}</span>` : ''}
            </div>
        ...
        <div class="passport-right">
          ...
          <div class="passport-tab-panel" id="pp-tab-details">
            <div class="form-grid">
              ...
              <label>Language(s)<input id="pp-language" type="text" value="${escapeHtml(f.language||'')}" placeholder="e.g. French, Arabic"></label>
              <label>Country(ies)<input id="pp-country" type="text" value="${escapeHtml(f.country||'')}" placeholder="e.g. France, Lebanon"></label>
              <label>Genre<input id="pp-genre" type="text" value="${escapeHtml(f.genre||'')}" placeholder="e.g. Drama, Thriller"></label>
              <label>Distributor<input id="pp-distributor" type="text" value="${escapeHtml(f.distributor||'')}" placeholder="e.g. Curzon, MUBI, BFI"></label>
              <label class="span-2">Where / When to See
                <div class="where-to-see-row">
                  <label class="where-option"><input type="checkbox" id="pp-wts-stream" ${hasStream?'checked':''}> Stream</label>
                  <label class="where-option"><input type="checkbox" id="pp-wts-disk" ${hasDisk?'checked':''}> Disk</label>
                  <label class="where-option"><input type="checkbox" id="pp-wts-screen" ${hasScreen?'checked':''}> Screen</label>
  ...
  $('#pp-save-status').onclick = async () => {
    ...
    if (newStatus === 'greenlist' && f.status !== 'greenlist') {
...
async function loadPassportVotes(filmId, overlay) {
  ...
  container.innerHTML = `
    ...
    <label style="margin-top:12px;display:block;text-transform:none;letter-spacing:0;font-size:13px;color:var(--ink-2);">
      Comment — editable anytime
...
async function loadPassportScreenings(filmId, overlay) {
  ...
  container.querySelector('#pp-sc-loc')?.addEventListener('change', async e => {
    if (e.target.value === '__add') {
      ...
      if (nl) await loadPassportScreenings(filmId, overlay);
      else e.target.value = '';
    }
  });
...
async function loadNextProg() {
  ...
  document.getElementById('btn-select-first9')?.addEventListener('click', () => {
    Array.from(list.querySelectorAll('input[data-select-film]')).forEach((cb,i) => cb.checked = i < 9);
  });
  document.getElementById('btn-clear-select')?.addEventListener('click', () => {
    list.querySelectorAll('input[data-select-film]').forEach(cb => cb.checked = false);
  });
...
async function loadDiscarded() {
  ...
  docs.forEach(doc => {
    const f = { id: doc.id, ...doc.data() };
    const reason = f.discardedReason
      ? `<div class="discard-reason">Reason: ${escapeHtml(f.discardedReason)}</div>`
      : '<div class="discard-reason">No reason — open Passport to add one.</div>';
...
async function loadArchive() {
  ...
  docs.forEach(doc => {
    const f = { id: doc.id, ...doc.data() };
    const prog = f.programmeDate ? `<span class="badge badge-muted" style="margin-top:6px;">Programme: ${f.programmeDate}</span>` : '';
    list.insertAdjacentHTML('beforeend', simpleFilmCard(f, prog));
  });
  attachPassportButtons(list);
}
...
async function fetchAddressesByPostcode(pc) {
  ...
  try {
    ...
    return data.map(item => {
      ...
      const postcode = a.postcode ? a.postcode.toUpperCase().replace(/\s+/g,'') : norm;
...
function showAddLocationModal(prefill = {}) {
  return new Promise(resolve => {
    ...
    const toast = msg => { const m = $('#loc-msg'); m.textContent = msg; m.classList.remove('hidden'); setTimeout(() => m.classList.add('hidden'), 2000); };
    ...
    $('#loc-lookup').onclick = async () => {
      ...
      results.forEach(a => {
        ...
        b.className = 'btn btn-ghost'; b.type = 'button'; b.style.cssText = 'width:100%;text-align:left;margin-bottom:4px;font-size:13px;';
        b.textContent = a.label;
        b.onclick = () => { $('#loc-house').value=a.house||''; $('#loc-street').value=a.street||''; $('#loc-town').value=a.town||''; $('#loc-county').value=a.county||''; $('#loc-postcode').value=a.postcode; listEl.innerHTML=''; };
    ...
    $('#loc-save').onclick = async () => {
      ...
      try {
        if (prefill.id) { await db.collection('locations').doc(prefill.id).update({ name, address:addr, postcode, city:town, house, street, town, county }); close({ id:prefill.id, name }); }
        else { const ref = await db.collection('locations').add({ name, address:addr, postcode, city:town, house, street, town, county, createdAt:firebase.firestore.FieldValue.serverTimestamp() }); close({ id:ref.id, name }); }
